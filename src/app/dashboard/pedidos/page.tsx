...
const NEXT_LABEL: Partial<Record<PedidoStatus, string>> = {
  ...
  em_andamento: 'Marcar como Faturado',
};
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-slate-500 text-sm">{pedidos.length} pedido(s)</p>
      </div>
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50 cursor-pointer" onClick={() => openDetail(p)}>
                  ...
                  <td className="px-6 py-3 text-slate-500">{p.pedido_itens?.length || 0}</td>
        ...
        <Modal open={!!selected} onClose={() => setSelected(null)} title={`Pedido #${selected.numero}`} size="xl">
          <div className="space-y-5">
                ...
                {STEPS.map((step, idx) => {
                  ...
                  return (
                    <div key={step.status} className="flex items-center gap-2 flex-1">
                      <div className={`flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg text-xs font-medium ${
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div><span className="text-slate-400">Data</span><p className="font-medium">{new Date(selected.created_at).toLocaleDateString('pt-BR')}</p></div>
            </div>
            ...
            {NEXT_STATUS[selected.status] && (
              <Button onClick={() => handleStatusChange(NEXT_STATUS[selected.status]!)} loading={acting} className="w-full">
