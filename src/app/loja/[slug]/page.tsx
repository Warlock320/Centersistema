...
export default async function CatalogoPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const corDark = cor + 'dd';
  const css = `
    body{margin:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f1f5f9}*{box-sizing:border-box}
    .lh{background:linear-gradient(135deg,${cor} 0%,${corDark} 100%);position:relative;overflow:hidden}
    .lh::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 80% 50%,rgba(255,255,255,.12) 0%,transparent 60%);pointer-events:none}
    .lhi{max-width:900px;margin:0 auto;padding:48px 24px 40px;display:flex;flex-direction:column;align-items:center;text-align:center;gap:16px;position:relative}
    .ll{width:100px;height:100px;border-radius:50%;object-fit:cover;border:4px solid rgba(255,255,255,.9);box-shadow:0 8px 32px rgba(0,0,0,.25),0 0 0 8px rgba(255,255,255,.15);flex-shrink:0}
    .llph{width:100px;height:100px;border-radius:50%;background:rgba(255,255,255,.2);border:4px solid rgba(255,255,255,.6);display:flex;align-items:center;justify-content:center;font-size:44px;box-shadow:0 8px 32px rgba(0,0,0,.2)}
    .lh h1{color:#fff;font-size:30px;font-weight:900;margin:0;letter-spacing:-.5px;text-shadow:0 2px 8px rgba(0,0,0,.2)}
    .lh p{color:rgba(255,255,255,.85);font-size:14px;margin:0;max-width:500px;line-height:1.6}
    .lhstats{display:flex;align-items:center;gap:10px;flex-wrap:wrap;justify-content:center;margin-top:4px}
    .lhbadge{background:rgba(255,255,255,.2);backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,.3);color:#fff;font-size:12px;font-weight:700;padding:6px 14px;border-radius:99px;white-space:nowrap}
    .lhwa{display:inline-flex;align-items:center;gap:8px;background:#22c55e;color:#fff;padding:10px 20px;border-radius:99px;font-size:13px;font-weight:700;text-decoration:none;transition:all .2s;box-shadow:0 4px 14px rgba(34,197,94,.4)}
    .lhwa:hover{background:#16a34a;transform:translateY(-1px);box-shadow:0 6px 20px rgba(34,197,94,.5)}
    ...
    @media(max-width:700px){.lhi{padding:36px 20px 32px}.lh h1{font-size:22px}.ll,.llph{width:80px;height:80px}.lgrid{grid-template-columns:repeat(2,1fr);gap:10px}.lcimg{height:135px}.lcbody{padding:10px}.lcprice{font-size:17px}.lprange{display:none}}
  `;
    ...
    <>
      ...
      <header className="lh">
        <div className="lhi">
          ...
          <div className="lhstats">
            <span className="lhbadge">📦 {total} produto{total !== 1 ? 's' : ''}</span>
            <span className="lhbadge">✅ {emEstoque} disponíve{emEstoque !== 1 ? 'is' : 'l'}</span>
            {emp.catalogo_whatsapp && (
              <a
                href={`https://wa.me/${emp.catalogo_whatsapp.replace(/\D/g, '')}`}
                target="_blank"
